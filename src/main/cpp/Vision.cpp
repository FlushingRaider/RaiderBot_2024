...
void VisionRun()
{
    ...
    for (unsigned camResultIndex = 0; camResultIndex < L_VisCamResults.size(); camResultIndex++)
    {
        ...
        // if we hit the last camera and none have had a target set best cam to an impossible number to show this
        if (L_VisCamResults[L_bestCam].second == std::numeric_limits<double_t>::max())
        {
            L_foundTargets = false;
        }
    }

    frc::SmartDashboard::PutNumber("Best cam", L_bestCam);
    frc::SmartDashboard::PutBoolean("found targets", L_foundTargets);
